...
| Scope | The coach declines to write code, essays, homework or anything else a general assistant is used for, including when the ask is wrapped in lifting ("a Python script to track my squat") or pressed with an outage elsewhere. Every reply is paid for per token by one operator, so a person using this as a substitute assistant spends that budget on work the product does not do. This is a prompt boundary rather than a computed gate — "is this message a coding request" is not something code can decide without a second model call — so it is held by two adversarial eval scenarios rather than by a unit test, and one of them exists to prove the boundary does NOT fire on somebody answering the coach's own question about what is getting in the way of their training. That false positive would be the more expensive failure. |
